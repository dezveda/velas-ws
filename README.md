...
- **Sub-millisecond OS Timers**: Adjusts Windows system timer resolution to 0.5ms (`NtSetTimerResolution` & `timeBeginPeriod(1)`).
...
- **Optimized Binary Ring Buffer**: Lock-free binary MPSC Ring Buffer with 64-byte slots for ultra-fast event ingestion.
...
- **Dynamic Leader/Standby Race Arbitration**: Dynamic race resolution for Kline, Ticker, Trade, and Order Book data. Under steady state, an EWMA model transitions lagging feeds to standby to cut mutex contention while keeping health ping/pongs active.
...
- **Footprint Charting & VDP**: Dynamic footprint bucket ticks calculated based on price order-of-magnitude, rendering micro-structure Buy/Sell volumes directly on candles.
...
- **Absorption Detection**: Highlighting aggressive order absorption on Ask/Bid levels with asymmetric debouncing.
...
- **Sci-Fi HUD & Classic Ticker Styles**: Zero-allocation in-place HSL color-mutating HUD ticker with velocity motion blur.
...
- **Z-Order Modes**: Toggle between Always-on-Top overlay and Desktop Wallpaper mode.
...
- **Z-Order Mode**: Always on Top or Wallpaper mode.
- **Timeframe**: Select from `1m`, `3m`, `5m`, `15m`, `30m`, `1h`, `2h`, `4h`, `6h`, `12h`, `1d`, `1w`, or `1M`.

### Controls
- **Right Click**: Opens the context menu to quickly change symbols or timeframes on the fly.
- **Mouse Hover**: Interactive evasion zone morphs rendering elements around the mouse cursor when running in Always on Top mode.
