...
- **Sub-millisecond OS Timers**: Adjusts Windows system timer resolution to 0.5ms via `NtSetTimerResolution` (with `timeBeginPeriod(1)` adjusting system timer period to 1ms).
...
- **Z-Order Modes**: Toggle between Always-on-Top overlay and Standard window mode (non-overlay).
...
- **Z-Order Mode**: Always on Top or Standard window mode.
